...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareRefresh() {
		...
		// Allow for the collection of early ApplicationEvents,（允许收集早期的申请事件，）
		// to be published once the multicaster is available...（一旦多播机可用就会发布…）
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 将用于检测内部bean的早期后处理器注册为 ApplicationListeners.
	...
	/**
	 * 实例化和调用所有注册的BeanPostProcessor bean，如果给定了显式的顺序。
	 * 必须在应用程序bean实例化之前调用。
	 */
	 ...
	 * 如果上下文中没有定义，则使用SimpleApplicationEventMulticaster(简单的应用程序事件多播)。
	 * @see SimpleApplicationEventMulticaster
	 */
	...
	/**
	 * 初始化LifecycleProcessor。如果上下文中没有定义，则使用DefaultLifecycleProcessor。
	 *
	 * @see DefaultLifecycleProcessor
	 */
	...
	protected void registerListeners() {
		...
		// 不要在这里初始化FactoryBean:
		// 		我们需要保持所有常规bean未初始化，以便让后处理程序应用它们!
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果以前没有bean后处理器(比如PropertyPlaceholderConfigurer bean)注册过任何bean
	...
	/**
	 * 调用LifecycleProcessor的onRefresh()方法并发布{@link ContextRefreshedEvent}，完成上下文的刷新。
	 */
	...
	/**
	 * 断言此上下文的BeanFactory当前是活动的，如果不是，抛出{@link IllegalStateException}。
	 * <p>被依赖于活动上下文的所有{@link BeanFactory}委托方法调用，特别是所有bean访问器方法。
	 * <p>默认实现检查{@link #isActive() 此上下文的“活动”}状态。
	 * 如果{@link #getBeanFactory()}本身在这种情况下抛出异常，则可能会被覆盖用于更具体的检查，或者用于无操作。
	 */
