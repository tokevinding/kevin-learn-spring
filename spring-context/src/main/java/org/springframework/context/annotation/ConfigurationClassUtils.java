...
abstract class ConfigurationClassUtils {
	...
	/**
	 * 检查给定的bean定义是否为配置类(或在配置/组件类中声明的嵌套组件类，也要自动注册)的候选类，并相应地对其进行标记。
	 *
	...
	public static boolean checkConfigurationClassCandidate(BeanDefinition beanDef, MetadataReaderFactory metadataReaderFactory) {
		...
		if (beanDef instanceof AnnotatedBeanDefinition &&
			...
			// 翻译：可以重用来自给定BeanDefinition的预解析元数据吗
		...
		// 它是一个完整的或lite配置候选…我们来确定它的阶值，如果有的话。
	 ...
	 * ，确定给定的bean定义是否表示一个完整的{@code @Configuration}类。
	 */
	...
	/**
	 * 通过检查{@link #checkConfigurationClassCandidate}的元数据标记
	 * ，确定给定的bean定义是否表示一个lite {@code @Configuration}类。
	 */
